...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        import sys
        ret = sys.maxint*1.0
        for selfNode in self.points:
            for otherNode in other.points:
                ret = min(ret, selfNode.distance(otherNode))
        return ret
    def maxLinkageDist(self, other):
        ...
        for selfNode in self.points:
            for otherNode in other.points:
                ret = max(ret, selfNode.distance(otherNode))
        return ret
    def averageLinkageDist(self, other):
        ...
        sumDis, count = 0.0, 0
        for selfNode in self.points:
            tmp = reduce(lambda x, y: (x[0]+y[0], x[1]+y[1]),
                          map(lambda x: (selfNode.distance(x),1), other.points)
                          )
            sumDis += tmp[0]
            count += tmp[1]
        return sumDis/count
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        mergedCluster = Cluster([q for q in c1.points] + [p for p in c2.points if p not in c1.points], c1.pointType)
    ...
    def findClosest(self, linkage):
        ...
        criteria = sys.maxint*1.0

        for combo in itertools.combinations(self.members, 2):
            tmp = linkage(combo[0],combo[1])
            if  tmp < criteria:
                criteria = tmp
                minTuple = combo
        return minTuple
    def mergeOne(self, linkage):
        ...
        if self.numClusters() >= 2:
            simiTuple = self.findClosest(linkage)
            self.mergeClusters(simiTuple[0], simiTuple[1])
            return simiTuple
